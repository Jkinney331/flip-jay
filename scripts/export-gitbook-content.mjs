...
// All the content organized by sections
...
- Console support`
  ...
  // Technology Stack - Update existing page
  'Technology Stack': {
    order: 6,
...
- Performance monitoring`
  ...
  // Case Studies - Update existing page
  'Case Studies': {
    order: 8,
...
function exportContentToFiles() {
  console.log('📝 Exporting GitBook content to organized files...');
  
  // Create output directory
  const outputDir = 'gitbook-content-export';
  if (!fs.existsSync(outputDir)) {
    fs.mkdirSync(outputDir);
  }

  // Create individual files for each section
  Object.entries(GITBOOK_CONTENT).forEach(([title, data]) => {
    const filename = `${data.order.toString().padStart(2, '0')}-${title.toLowerCase().replace(/[^a-z0-9]/g, '-')}.md`;
...
${data.content}`;
  ...
  // Create a master file with all content
  const masterContent = Object.entries(GITBOOK_CONTENT)
    .sort(([,a], [,b]) => a.order - b.order)
    .map(([title, data]) => `# ${title}\n\n${data.content}`)
    .join('\n\n---\n\n');

  fs.writeFileSync(path.join(outputDir, '00-master-content.md'), masterContent);
  console.log('✅ Created: 00-master-content.md (complete content file)');

  // Create a mapping file showing existing GitBook pages
  const mappingContent = `# GitBook Page Mapping
...
`;
  ...
  fs.writeFileSync(path.join(outputDir, 'page-mapping.md'), mappingContent);
  console.log('✅ Created: page-mapping.md (instructions for updating GitBook)');

  console.log(`\n🎉 Content export complete!`);
  console.log(`📁 Files created in: ${outputDir}/`);
  console.log(`📊 Total sections: ${Object.keys(GITBOOK_CONTENT).length}`);
  console.log(`\n📋 Next steps:`);
  console.log(`1. Open your GitBook: https://jays-personal-organization-1.gitbook.io/flip-tech-pro/`);
  console.log(`2. Copy content from the numbered files into the corresponding GitBook pages`);
  console.log(`3. Use page-mapping.md to see which existing pages to update`);
  console.log(`4. Create new pages for content not already in GitBook`);
}

exportContentToFiles();
